...
class ResqueScheduler
{
	...
	public static function enqueueAt($at, $queue, $class, $args = array())
	{
		...
		return Resque_Event::trigger('afterSchedule', array(
			'at'    => $at,
			'queue' => $queue,
			'class' => $class,
			'args'  => $args,
    ...
    public static function removeDelayed($queue, $class, $args)
    {
       ...
       foreach($redis->keys('delayed:*') as $key)
       {
           ...
           $destroyed+=$redis->lrem(intval($key),0,$item);
       }
	...
	public static function nextDelayedTimestamp($at = null)
	{
        ...
        // Fix for using php-redis than Credis or Redisent.
		// $items = Resque::redis()->zrangebyscore('delayed_queue_schedule', '-inf', $at, 'LIMIT', 0, 1);
        $items = Resque::redis()->zrangebyscore('delayed_queue_schedule', '0', $at, array('limit' => array(0, 1)));        
